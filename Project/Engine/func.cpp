...
const float& GetSmoothDir(CGameObject* _pStartObject 
	, CGameObject* _pTargetObj, const float& _degree)
{
	const Vec3& vOwnerPos = _pStartObject->Transform()->GetWorldPos();
	const Vec3& vTargetPos = _pTargetObj->Transform()->GetWorldPos();
	const Vec3& vPrevDir = _pStartObject->Transform()->GetRelativeRot();
	float PrevDir = vPrevDir.y;
	const float& Rot = GetDir(vOwnerPos, vTargetPos);
...
const float& GetSmoothDir(Vec3 _vStart, Vec3 _vTarget, Vec3 _vPrevDir, float _degree)
{
